    ...
    public function signUp($request)
    {
        try {
            ...
            if ($user_data == 0) {
                ...
                if ($request->account_type == $this->userModel::NORMAL_SIGNUP) {
                    $response = $this->loginHelper->sendEmail($this->emailTemplateModel::ACCOUNT_VERIFICATION, $request, $user_data['otp']);
                    ...
                    if ($response['code'] == 200) {
                        ...
                        return ['message' => $response['message'], 'response' => 422];
                    }
                } else {
                ...
                return ['message' => trans("api_message.username_already_exist"), 'response' => 422];
            }
        } catch (\Exception $e) {
            return ['message' => $e->getMessage(), 'response' => 422];
        }
    }
            ...
            if ($request->account_type == $this->userModel::NORMAL_SIGNUP) {
                ...
                if (!empty($user_data)) {
                    if ($user_data->is_active == $this->userModel::IS_INACTIVE && $user_data->deactivated_by == '1') {
                        return ['message' => trans('api_message.inactive_account_error'), 'response' => 400, 'data' => ['is_active' => $user_data->is_active, 'deactivated_by' => $user_data->deactivated_by]];
                    }

                    if ($user_data->is_email_verified == '0') {
                        $user_data->user_otp = 1;
                        $user_data->save();
                        $this->loginHelper->sendEmail($this->emailTemplateModel::ACCOUNT_VERIFICATION, $user_data, $user_data->otp);
                    ...
                    if (!empty($user_data->password) && empty($user_data->account_type_id)) {
                        if (!Hash::check($request->password, $user_data->password)) {
                            return ['message' => trans('api_message.invalid_login_details'), 'response' => 400, 'user_status' => ''];
                        }
                    } else {
                        if (!empty($user_data->account_type_id)) {
                            return ['message' => str_replace('{{SOCIAL}}', Config::get('constants.account_type')[$user_data->account_type], ' Sign up from social'), 'response' => 400, 'user_status' => ''];
                        }
                    }
                    ...
                    if ($login_data['code'] == 200) {
                        ...
                        return ['message' => trans("api_message.something_went_wrong"), 'response' => 422];
                    }
                } else {
                    return ['message' => trans("api_message.no_account_found"), 'response' => 400];
                }
            }
        } catch (\Exception $e) {
            return ['message' => $e->getMessage(), 'response' => 422];
    ...
    public function verifyOtp($request)
    {
        try {
            ...
            if (!empty($user_data)) {
                ...
                if ($diff > 0) {
                    return ['message' => trans("api_message.otp_expired"), 'response' => 422];
                } else {
                    $login_data = $this->loginHelper->checkLogin($user_data, $request);
                    $token = $user_data->createToken('auth_token')->plainTextToken;
                    $user_data->otp = '';
                ...
                return ['message' => trans("api_message.invalid_otp"), 'response' => 422];
            }
        } catch (\Exception $e) {
            return ['message' => $e->getMessage(), 'response' => 422];
        }
    ...
    public function resendOtp($request)
    {
        try {
            ...
            if (!empty($user_data)) {
                ...
                return ['message' => trans("api_message.user_not_found"), 'response' => 422];
            }
        } catch (\Exception $e) {
            return ['message' => $e->getMessage(), 'response' => 422];
        }
        ...
        try {
            ...
            if (!empty($user_data)) {
                if (!empty($user_data->social_id)) {
                    return ['message' => str_replace('{{SOCIAL}}', Config::get('constants.account_type')[$user_data->account_type], "{{SOCIAL}} Social forget password"), 'response' => 422];
                } else {
                    $user_data->user_otp = 1;
                    $user_data->save();
                ...
                return ['message' => trans("api_message.no_account_found_with_email"), 'response' => 422];
            }
        } catch (\Exception $e) {
            return ['message' => $e->getMessage(), 'response' => 422];
        }
    ...
    public function resetPassword($request)
    {
        try {
            ...
            if (!Hash::check($request->password, $user_data->password)) {
                ...
                return ['message' => trans("api_message.old_and_new_not_same"), 'response' => 422];
            }
        } catch (\Exception $e) {
            return ['message' => $e->getMessage(), 'response' => 422];
        }
